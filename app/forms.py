...
class FormCreatePlaylist(Form):
    id = StringField('id', validators=[DataRequired(), Length(min=2, max=50)])
    password = StringField('password', validators=[Length(max=50)])

    def validate(self):
        if not Form.validate(self):
            return False
        if models.Playlist.query.filter_by(id=self.id.data).all():
            self.id.errors.append('Playlist id already used')
            return False
        return True
...
class FormAddVideo(Form):
    ...
    def validate(self):  # Cannot check if video exist on youtube
        if not Form.validate(self):
            return False
        if models.Video.query.filter_by(ytid=extract_ytid(str(self.url.data)), playlist_id=self.playlist.id).all():
